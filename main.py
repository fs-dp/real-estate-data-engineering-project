...
SBR_WS_CDP = 'wss://brd-customer-hl_a3a1dd24-zone-real_estate_browser:ip0z1uotrr6f@brd.superproxy.io:9222'
...
def extract_property_details(input):
    ...
    command = """
        You are a data extractor model and you have been tasked with extracting information for me into json. 
...
def extract_floor_plan(soup):
    ...
    plan = {}
    floor_plan = soup.find('div', {"data-tested": "floorplan-thumbnail-0"})
    if floor_plan:
...
async def run(pw):
    ...
    try:
        ...
        #enter London in the saerch bar and press enter to search
        ...
        for idx, div in enumerate(soup.find_all(name="div", class_="dkr2t82")):
            ...
            property_details = soup.select_one('div[class="_14bi3x32f"]')
